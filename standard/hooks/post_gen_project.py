...
def run(msg, cmd_arr, stderr2out=False, print_output=False):
    ...
    if stderr2out is False:
        ...
        output = sub.run(
            cmd_arr, stdout=sub.PIPE, stderr=sub.STDOUT, shell=shell_
...
def write_pythonpath_vscode(
    pypath, vscode_config_path=".vscode/settings.json"
):
...
pre_commit_install = run(
    "Installing pre-commit", [venv_python, "-m", "pre_commit", "install"]
)
pre_commit_autoupdate = run(
    "Auto Updating pre-commit", [venv_python, "-m", "pre_commit", "autoupdate"]
)
git_init = run("Adding project files to GIT repo", ["git", "add", "."])
git_commit = run(
    "Committing to GIT repo",
    ["git", "commit", "-m", "Initialize environment"],
    stderr2out=True,
)
